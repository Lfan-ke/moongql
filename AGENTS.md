...
- There is no shutdown API here, deliberately. A subscription is drained at `subscribe` time and completed in the same batch, so no operation is ever live between two client messages and nothing outlives a request. A client's `complete` is accepted and does nothing for the same reason. Streaming a subscription over time — a real event source rather than a materialised list — is what would create something to drain, and that change comes first.
